...
urdf = "talos/talos_description/robots/talos_reduced_no_hands.urdf"
path = "talos/talos_description/meshes/../.."
...
q0= np.hstack([np.array([0, 0, 0.9, 0, 0, 0, 1]), np.zeros(na)])

q1 = np.zeros(na)
q1[0:6] = np.array([0, 0, -0.44, 0.9, -0.45, 0])
q1[6:12] = np.array([0, 0, -0.44, 0.9, -0.45, 0])
q1[14:21] = np.array([0, -0.24, 0, -1, 0, 0, 0])
q1[21:28] = np.array([0, -0.24, 0, -1, 0, 0, 0])
...
no_sim_per_mpc = int(round(dt_mpc / dt))        # number of sim between mpc update 100
no_sim_per_step = int(round(step_dur / dt))     # number of sim between foot steps 0.8/0.001 = 800
